...
!        calculation of trial yield/flow stress
         ...
         sigma_pc = M(i_material) * b(i_material) * G_alpha_c * sqrt(rho_0(i_material) * rho_hat_ci)   ! {Eq. 140}
         sigma_pw = M(i_material) * b(i_material) * G_alpha_w * sqrt(rho_0(i_material) * rho_hat_wi)   ! {Eq. 140}
         sigma_y  = sigma_v + sigma_pc + sigma_pw   ! {Eq. 139}
     else
...
!        calculation of yield/flow stress
         sigma_v  = sigma_v00(i_material) * (1.0e0 + r_v(i_material) * (T_hat_n - 1.0e0)**s_v(i_material)) * (delta_eps_bar_p / delta_t / eps_bar_dot_0(i_material))**m_v   ! {Eq. 134}
...
!        calculation of viscoplastic tangent modulus   {Eqs. 149, 150, 151}
