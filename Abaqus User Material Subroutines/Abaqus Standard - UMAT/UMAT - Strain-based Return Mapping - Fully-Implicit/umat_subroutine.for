...
#include 'matrix_inverse_subroutine.f90'
...
!    declaration of subroutines' local variables
     ...
     real(pReal) :: &
         ...
         sigma_p, &                                                      ! plastic/critical stress [MPa]
         ...
         norm_R, &                                                       ! normalized norm of residual functions matrix in multivariate return mapping using newton-raphson method [-]
     ...
     real(pReal), dimension(4) :: &
         x, &                                                            ! target variable    matrix to be computed using the newton-raphson method
         R                                                               ! residual functions matrix to be solved   using the newton-raphson method

     real(pReal), dimension(4,4) :: &
         dR_dx, &                                                        ! jacobian matrix of multivariate return mapping
         dR_dx_inv                                                       ! inverse of Jacobian matrix of multivariate return mapping
...
!    initializing microstructural state variables (different types of dislocation density)   {Eq. Box 1.5(c)}
     rho_hat_cm = rho_hat_cm_n
     rho_hat_ci = rho_hat_ci_n
     rho_hat_wi = rho_hat_wi_n

!    calculation of trial plastic stress   {Eq. 111}
     call plasticity(i_material, delta_t, T_hat_n, delta_eps_bar_p, rho_hat_cm_n, rho_hat_ci_n, rho_hat_wi_n, rho_hat_cm, rho_hat_ci, rho_hat_wi, sigma_p, H_p, beta, R, dR_dx)
     sigma_y       = sigma_p
...
!        initializing variables for multivariate return mapping
         delta_eps_bar_p = delta_t * eps_bar_dot_p   ! {Eq. 112}
         call plasticity(i_material, delta_t, T_hat_n, delta_eps_bar_p, rho_hat_cm_n, rho_hat_ci_n, rho_hat_wi_n, rho_hat_cm, rho_hat_ci, rho_hat_wi, sigma_p, H_p, beta, R, dR_dx)
         x(1) = delta_eps_bar_p
         x(2) = rho_hat_cm
         x(3) = rho_hat_ci
...
!        multivariate return mapping loop: solving for equivalent plastic strain increment using newton-raphson iterative method
         do k_NR = 1, k_NR_max
             call plasticity(i_material, delta_t, T_hat_n, delta_eps_bar_p, rho_hat_cm_n, rho_hat_ci_n, rho_hat_wi_n, rho_hat_cm, rho_hat_ci, rho_hat_wi, sigma_p, H_p, beta, R, dR_dx)
             sigma_y = sigma_bar_trial - 3.0e0 * G * delta_eps_bar_p                           ! {Eq. 109(c)}
             sigma_v = sigma_y - sigma_p                                                       ! {Eq. 109(b)}
             eps_bar_dot_p = eps_bar_dot_0(i_material) * (sigma_v / sigma_v0)**(1.0e0 / m_v)   ! {Eq. 109}

!            completion of residual functions matrix and jacobian matrix in Return mapping using newton-raphson method
             R(1)       = delta_eps_bar_p - delta_t * eps_bar_dot_p   ! {Eq. 110}
             dR_dx(1,1) = 1.0e0 + 3.0e0 * G / (m_v * sigma_v) * delta_t * eps_bar_dot_p
             dR_dx(1,2) = 0.0e0
             dR_dx(1,3) = dR_dx(1,3) / (m_v * sigma_v) * delta_t * eps_bar_dot_p
             dR_dx(1,4) = dR_dx(1,4) / (m_v * sigma_v) * delta_t * eps_bar_dot_p

!            checking for newton-raphson iteration stop criterion
             norm_R = sqrt((R(1) / (delta_t * xi_mean * eps_bar_dot_0(i_material)))**2 + (R(2) / rho_hat_cm_n)**2 + (R(3) / rho_hat_ci_n)**2 + (R(4) / rho_hat_wi_n)**2)
             if (norm_R < chi) exit   ! {Eq. 113}

!            updating newton-raphson variables (equivalent plastic strain increment and different types of dislocation density)
             call matrix_inverse(dR_dx,dR_dx_inv,4)
             x = x - matmul(dR_dx_inv,R)
             delta_eps_bar_p = x(1)
             rho_hat_cm      = x(2)
             rho_hat_ci      = x(3)
             rho_hat_wi      = x(4)
...
!        calculation of effective moduli
         ...
         H_vp  = m_v / delta_eps_bar_p * sigma_v + H_p                      ! {Eqs. 106, 107}
