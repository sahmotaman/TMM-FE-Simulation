...
!            retrieving state variables at the beginning of the time increment (end of the last time step) [-]
             rho_hat_cm_n    = sv_n(i_mp,1) / rho_0(i_material)   ! {Eq. 84(b,c)}
             rho_hat_ci_n    = sv_n(i_mp,2) / rho_0(i_material)   ! {Eq. 84(b,c)}
             rho_hat_wi_n    = sv_n(i_mp,3) / rho_0(i_material)   ! {Eq. 84(b,c)}
...
!        calculation of plastic/critical stress
         G_alpha_c = G_0(i_material) * alpha_c0(i_material) * (1.0e0 + r_G_alpha_c(i_material) * (T_hat_n - 1.0e0)**s_G_alpha_c(i_material))   ! {Eq. 133}
         G_alpha_w = G_0(i_material) * alpha_w0(i_material) * (1.0e0 + r_G_alpha_w(i_material) * (T_hat_n - 1.0e0)**s_G_alpha_w(i_material))   ! {Eq. 133}
         sigma_p_n = M(i_material) * b(i_material) * G_alpha_c * sqrt(rho_0(i_material) * rho_hat_ci_n) + M(i_material) * b(i_material) * G_alpha_w * sqrt(rho_0(i_material) * rho_hat_wi_n)   ! {Eqs. 131 and 132}
...
!        calculation of equivalent plastic strain increment
         ...
         eps_bar_dot_p   = eps_bar_dot_0(i_material) * (sigma_v_n / sigma_v0)**(1.0e0 / m_v)                   ! {Eq. 152}
...
!        calculation of plastic and elastic strain increment tensors
         if (sigma_bar_n == 0.0e0) then
             ...
             delta_eps_p = 3.0e0 / 2.0e0 * sigma_d_n / sigma_bar_n * delta_eps_bar_p   ! {Eq. 122 (b)}
         end if
         delta_eps_e = delta_eps(i_mp,:) - delta_eps_p   ! {Eq. 26}
...
!        calculation of total specific elasto-plastic work
         ...
         w(i_mp) = w_n(i_mp) + delta_w   ! {Eq. 105}

!        calculation of plastic dissipation work and plastic heat generation
         beta      = (2.0e0 * (del_rho_hat_an_cm_n + del_rho_hat_an_ci_n + del_rho_hat_an_wi_n) / del_rho_hat_gn_cm_n)**kappa(i_material)   ! {Eq. 98}
         q_p(i_mp) = q_p_n(i_mp) + beta * sigma_bar_n * delta_eps_bar_p / rho_mass(i_mp)   ! {Eqs. Box 1.9, 105}

!        calculation of equivalent plastic strain rate and equivalent plastic strain
         eps_bar_p = eps_bar_p_n + delta_eps_bar_p   ! {Eq. 105}

!-----------------------------------------------------------------------
! post-processing
!-----------------------------------------------------------------------
!        updating state variables
         sv(i_mp,1) = rho_hat_cm * rho_0(i_material)   ! {Eq. 84(b,c)}
         sv(i_mp,2) = rho_hat_ci * rho_0(i_material)   ! {Eq. 84(b,c)}
         sv(i_mp,3) = rho_hat_wi * rho_0(i_material)   ! {Eq. 84(b,c)}
         sv(i_mp,4) = eps_bar_p
