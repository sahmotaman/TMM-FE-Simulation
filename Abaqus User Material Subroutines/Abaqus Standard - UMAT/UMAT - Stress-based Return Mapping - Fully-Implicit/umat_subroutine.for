...
!    declaration of subroutines' local variables
     ...
     integer(pInt) :: &
         ...
         i, &                                                            ! loop index variable i
         j, &                                                            ! loop index variable j
...
!    calculation of corrected equivalent strain rate   {Eq. 141}
...
!    calculation of trial plastic stress   {Eq. 139}
     trial_flag = .true.
     call viscoplasticity(trial_flag, i_material, delta_t, T_hat_n, eps_bar_dot_p_corr, delta_eps_bar_p, rho_hat_cm_n, rho_hat_ci_n, rho_hat_wi_n, rho_hat_cm, rho_hat_ci, rho_hat_wi, sigma_y, H_vp, beta, R, dR_dx)
...
!        multivariate return mapping loop: solving for equivalent plastic strain increment and state variables using newton-raphson iterative method
         do k_NR = 1, k_NR_max
             call viscoplasticity(trial_flag, i_material, delta_t, T_hat_n, eps_bar_dot_p_corr, delta_eps_bar_p, rho_hat_cm_n, rho_hat_ci_n, rho_hat_wi_n, rho_hat_cm, rho_hat_ci, rho_hat_wi, sigma_y, H_vp, beta, R, dR_dx)

!            completion of residual functions matrix and jacobian matrix in Return mapping using newton-raphson method
             R(1)       = sigma_bar_trial - sigma_y - 3.0e0 * G * delta_eps_bar_p   ! {Eq. 138}
             dR_dx(1,1) = dR_dx(1,1) - 3.0e0 * G

!            checking for newton-raphson iteration stop criterion
             norm_R = sqrt((R(1) / sigma_y_trial)**2 + (R(2) / rho_hat_cm_n)**2 + (R(3) / rho_hat_ci_n)**2 + (R(4) / rho_hat_wi_n)**2)
             if (norm_R < chi) exit   ! {Eq. 146}

!            updating newton-raphson variables
